...
}qml_root/org/kde/plasma/core/qmldir"
                ksvg_core="${'
            """.trimIndent(),
            timeoutSeconds = 900,
            rootfs = paths.rootfs,
...
}plasma_core" ] || needs_qml_repair=1
                [ -f "${'
            """.trimIndent(),
            timeoutSeconds = 900,
            rootfs = paths.rootfs,
...
}needs_qml_repair" = 1 ]; then
                    rm -f /var/lib/dpkg/status-old /var/lib/dpkg/lock /var/lib/dpkg/lock-frontend
                    apt-get update
...
}qml_root/org/kde/plasma/core/libcorebindingsplugin.so"
                test -r "${'
            """.trimIndent(),
            timeoutSeconds = 900,
            rootfs = paths.rootfs,
...
}qml_root/org/kde/ksvg/libcorebindingsplugin.so"
